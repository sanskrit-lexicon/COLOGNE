...
if __name__=="__main__":
 ...
 recs = [Extra(line) for line in lines]
 for rec in recs:
  adjust_rec(rec)
 newlines = [rec.newline for rec in recs]
 recs_ok = [rec for rec in recs if rec.status]
 lines_ok = [rec.newline for rec in recs_ok]
 write_lines(fileout,lines_ok)
 
 lines_ok1 = [rec.method + ': ' + rec.newline for rec in recs_ok]
 write_lines(fileout1,lines_ok1)
 recs1 = [rec for rec in recs if not rec.status]
 lines1 = [rec.line for rec in recs1]
 fileout1 = 'temp_problem.txt'
