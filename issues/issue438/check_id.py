...
def process_xml(xml_file, x, output_md_file):
    ...
    for h1 in h1_entries:
        ...
        for ls in ls_tags:
            ls_data.append((L, key1, pc, ls.attrib["id"]))
    
    # Select X random ls entries
    selected_ls = random.sample(ls_data, min(x, len(ls_data)))
    
    # Generate Markdown content
    md_content = "| L | Key1 | ID | yes/no |\n"
    md_content += "|---|------|----|-------|\n"
    for L, key1, pc, ls_id in selected_ls:
        md_content += f"| {L} | [{key1}](https://www.sanskrit-lexicon.uni-koeln.de/scans/csl-apidev/servepdf.php?dict=PWG&page={pc}) | {ls_id} |  |\n"
